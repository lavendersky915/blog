...
define("mysql_database", default="IPPlusDB", help="blog database name")
...
class Lavender_STPI(BaseHandler):
    def get(self):
        ...
        while pages < 20:
            ...
            url = "https://www.googleapis.com/customsearch/v1?q="+keyword+"&start="+startindex+"&key=AIzaSyCgRHK1Q03VBAIH313mrsWVCdjHnTzNUKw&cx=005971756043172606388:edll3ji0ejq"
            result = urllib.urlopen(url).read()
            count = result.count('kind') - 1
            obj_result = tornado.escape.json_decode(result)
        ...
        pass
        length = len(decom)
        data = tornado.escape.json_encode(two)
        self.write(two)
